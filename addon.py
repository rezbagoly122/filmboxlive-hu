...
def login():
    ...
    if not username or not password:
        xbmcgui.Dialog().ok('Filmbox Live', 'Adja meg a felhasználói azonosítót a beállításokban.')
        return False
...
def list(search_phrase=None):
    ...
    next = int(page) + 1
    if next <= total:
        add_folder('[B]Következő oldal >>[/B]', resources+'transparent.png', {'mode': 'list', 'page': str(next), 'searchName': search_name})
