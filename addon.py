...
##############################################################################
# itt lehet megnézni az érvényes stream-eket
#channels_url ='http://www.filmboxliveapp.net/channel/channels_hun.json'
...
def login():
    ...
    if not username or not password:
        xbmcgui.Dialog().ok('Filmbox Live', 'Add meg a felhasználói azonosítót a beállításokban.')
        return False
    data = {
        'username': username,
        'password': password,
    ...
    if user_info.get('guest', '1') == '0':
        ...
        if not repeated:
            ...
            xbmcgui.Dialog().ok('Sikertelen bejelentkezés.',
                                'Ellenőrizze a felhasználónevet és a jelszót a plugin beállításaiban és az előfizetés érvényességét.')
        return False
...
##############
	if title == 'CASSANDRE HD':
	   add_item('CASSANDRE', image, True, False, {'mode': 'list', 'searchName': 'CASSANDRE'}, info_labels)
	   continue
	if title == 'VEUM':
	   add_item('VEUM', image, True, False, {'mode': 'list', 'searchName': 'VEUM'}, info_labels)
	   continue
	if title == 'FARKAS-PATAK HD':
	   add_item(title, image, True, False, {'mode': 'list', 'searchName': 'WOLF1'}, info_labels)
	   continue
	if title == 'EINSTEIN REJTÉLYEI HD':
	   add_item(title, image, True, False, {'mode': 'list', 'searchName': 'EINSTEIN1'}, info_labels)
	   continue
	if title == 'CSIKORGÓ ACÉL HD':
	   add_item(title, image, True, False, {'mode': 'list', 'searchName': 'METAL1'}, info_labels)
	   continue
	if title == 'BÁRÓK ÉS DÍLEREK HD':
	   add_item(title, image, True, False, {'mode': 'list', 'searchName': 'CARTEL1'}, info_labels)
	   continue
	if title == 'GYILKOS NAP HD':
	   add_item(title, image, True, False, {'mode': 'list', 'searchName': 'SUN1'}, info_labels)
	   continue
	if title == 'SZÖRNYETEG HD':
	   add_item(title, image, True, False, {'mode': 'list', 'searchName': 'MONSTER1'}, info_labels)
	   continue
	if title == 'BŰN ÉS ÁRTATLANSÁG HD':
	   add_item(title, image, True, False, {'mode': 'list', 'searchName': 'INNICENT1'}, info_labels)
	   continue
	if title == 'MÓDUSZ HD':
	   add_item(title, image, True, False, {'mode': 'list', 'searchName': 'MODUS1'}, info_labels)
	   continue
	if title == 'A MÓKA KEDVÉÉRT HD':
	   add_item(title, image, True, False, {'mode': 'list', 'searchName': 'FORFUN1'}, info_labels)
	   continue
	if title == 'ÁLOMUTAZÁS HD':
	   add_item(title, image, True, False, {'mode': 'list', 'searchName': 'JOURNEY1'}, info_labels)
	   continue
	if title == 'ÍGY KÉSZÜL HD':
	   add_item(title, image, True, False, {'mode': 'list', 'searchName': 'MADE1'}, info_labels)
	   continue
	if title == 'MUMU TRIBE HD':
	   add_item(title, image, True, False, {'mode': 'list', 'searchName': 'MUMU1'}, info_labels)
	   continue
	else:
    ...
    if next <= total:
##########################
        add_folder('[B]Ugrás a(z) '+str(int(next))+'. oldalra >> Összes oldal:'+str(int(total))+'[/B]', resources+'Kovetkezo.png', {'mode': 'list', 'page': str(next), 'searchName': search_name})
##########################
...
##########################
    genres = attributes.get('genres_hu', '').encode('utf-8', 'ignore').split(',')
##########################
    ...
    return {
########################
        'plot': attributes.get('description_hu', '').encode('utf-8', 'ignore'),
########################
        'duration': duration,
        'genre': genres,
...
def seasons():
    ...
    for season in seasons:
        add_folder('Évad ' + season, '', {'mode': 'list', 'id': id, 'season': season})
    xbmcplugin.endOfDirectory(addon_handle)
...
def home():
    if not userid:
        if not login():
            return
    add_folder('Filmek, sorozatok keresése...', resources + 'search.png', {'mode': 'search'})
