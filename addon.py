...
MUA = 'Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:69.0)\
         Gecko/20100101 Firefox/69.0'
publisher_id = '25131'
package_id = '120'
ticket_key = '5dd3e0da5423de187d9938a45e2c81b7'
message1 = 'Wrong username or password'
message2 = 'Session expired or multiple logins detected. Please login again.'
...
def login():
    ...
    if not username or not password:
        xbmcgui.Dialog().ok('Filmbox Live', 'Add meg a felhasználói azonosítót\
 a beállításokban.')
        addon.openSettings()
        return login()
    data = {
    ...
    if info == message1:
        xbmcgui.Dialog().ok('Sikertelen bejelentkezés.',
                            'Ellenőrizd: Felhasználó/Jelszó, Előfizetés érvényessége')
        addon.openSettings()
        return login()
    elif info == message2:
        return login()
    else:
        userid = response['response']['result']['user_info']['id']
        sessionid = response['response']['result']['user_info']['session_id']
        return True
...
##############
    ...
    if next <= total:
        ...
        add_folder('[B]Ugrás a(z) '+str(int(next))+'. oldalra >> Összes oldal:'+str(int(total))+'[/B]',
                   resources+'Kovetkezo.png', {'mode': 'list',
                    'page': str(next), 'searchName': search_name})
##########################
...
def prepare_info_labels(attributes):
    ##########################
    genres = attributes.get('genres_hu', '').encode(
        'utf-8', 'ignore').split(',')
##########################
    genres = map(lambda x: x.replace('1', '').replace(
        '2', '').replace('3', ''), genres)
